 ...
 *
 * 1) When the client is not running messages are stored in a persistent store that implements the MqttClientPersistent Interface. The default is
 * MqttDefaultFilePersistencew which stores messages safely across failures and system restarts. If no persistence is specified there is a fall back to
 * MemoryPersistence which will maintain the messages while the Mqtt client is instantiated.
 *
 * 2) When the client or specifically ClientState is instantiated the messages are read from the persistent store into: - outboundqos2 hashtable if a QoS 2
 * PUBLISH or PUBREL - outboundqos1 hashtable if a QoS 1 PUBLISH (see restoreState)
 *
 * 3) On Connect, copy messages from the outbound hashtables to the pendingMessages or pendingFlows vector in messageid order. - Initial message publish goes
 * onto the pendingmessages buffer. - PUBREL goes onto the pendingflows buffer (see restoreInflightMessages)
 *
 * 4) Sender thread reads messages from the pendingflows and pendingmessages buffer one at a time. The message is removed from the pendingbuffer but remains on
 * the outbound* hashtable. The hashtable is the place where the full set of outstanding messages are stored in memory. (Persistence is only used at start up)
 *
 * 5) Receiver thread - receives wire messages: - if QoS 1 then remove from persistence and outboundqos1 - if QoS 2 PUBREC send PUBREL. Updating the
 * outboundqos2 entry with the PUBREL and update persistence. - if QoS 2 PUBCOMP remove from persistence and outboundqos2
 *
 * Notes: because of the multithreaded nature of the client it is vital that any changes to this class take concurrency into account. For instance as soon as a
 * flow / message is put on the wire it is possible for the receiving thread to receive the ack and to be processing the response before the sending side has
 * finished processing. For instance a connect may be sent, the conack received before the connect notify send has been processed!
 *
 */
public class ClientState {
     ...
     *
     * @param list the list containing the messages to produce a new reordered list for - this will not be modified or replaced, i.e., be read-only to this
     * method
    ...
    /**
     * Submits a message for delivery. This method will block until there is room in the inFlightWindow for the message. The message is put into persistence
     * before returning.
     *
    ...
    /**
     * Check and send a ping if needed and check for ping timeout. Need to send a ping if nothing has been sent or received in the last keepalive interval. It
     * is important to check for both sent and received packets in order to catch the case where an app is solely sending QoS 0 messages or receiving QoS 0
     * messages. QoS 0 message are not good enough for checking a connection is alive as they are one way messages.
     *
    ...
    /**
     * This returns the next piece of work, ie message, for the CommsSender to send over the network. Calls to this method block until either: - there is a
     * message to be sent - the keepAlive interval is exceeded, which triggers a ping message to be returned - {@link ClientState#disconnected(MqttException)}
     * is called
     *
...
//		if (quiescing && actualInFlight == 0 && pendingFlows.size() == 0 && inFlightPubRels == 0 && callback.isQuiesced()) {
    ...
    /**
     * Called when waiters and callbacks have processed the message. For messages where delivery is complete the message can be removed from persistence and
    ...
    /**
     * Called during shutdown to work out if there are any tokens still to be notified and waiters to be unblocked. Notifying and unblocking takes place after
     * most shutdown processing has completed. The tokenstore is tidied up so it only contains outstanding delivery tokens which are valid after reconnect (if
    ...
    /**
     * Quiesce the client state, preventing any new messages getting sent, and preventing the callback on any newly received messages. After the timeout
     * expires, delete any pending messages except for outbound ACKs, and wait for those ACKs to complete.
     *
