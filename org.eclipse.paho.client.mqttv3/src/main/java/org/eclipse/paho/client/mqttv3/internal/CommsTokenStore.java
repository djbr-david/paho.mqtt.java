 ...
 *
 * All rights reserved. This program and the accompanying materials are made available under the terms of the Eclipse Public License v1.0 and Eclipse
 * Distribution License v1.0 which accompany this distribution.
 *
 * The Eclipse Public License is available at http://www.eclipse.org/legal/epl-v10.html and the Eclipse Distribution License is available at
 * http://www.eclipse.org/org/documents/edl-v10.php.
 *
 * Contributors: Dave Locke - initial API and implementation and/or initial documentation
 */
...
/**
 * Provides a "token" based system for storing and tracking actions across multiple threads. When a message is sent, a token is associated with the message and
 * saved using the {@link CommsTokenStore#saveToken(MqttToken, MqttWireMessage)} method. Anyone interested in tacking the state can call one of the wait methods
 * on the token or using the asynchronous listener callback method on the operation. The {@link CommsReceiver} class, on another thread, reads responses back
...
public class CommsTokenStore {
    ...
    /**
     * Restores a token after a client restart. This method could be called for a SEND of CONFIRM, but either way, the original SEND is what's needed to
     * re-build the token.
     *
