...
public class NetworkModuleService {
     ...
     * @throws IllegalArgumentException is case the URI is invalid or there is no {@link NetworkModule} installed for the URI scheme
     */
    ...
    /**
     * Java does URI parsing according to RFC2396 and thus hostnames are limited to alphanumeric characters and '-'. But the current &quot;Uniform Resource
     * Identifier (URI): Generic Syntax&quot; (RFC3986) allows for a much wider range of valid characters. This causes Java to fail parsing the authority part
     * and thus the user-info, host and port will not be set on an URI which does not conform to RFC2396.
     * <p>
     * This workaround tries to detect such a parsing failure and does tokenize the authority parts according to RFC3986, but does not enforce any character
     * restrictions (for sake of simplicity).
     *
    ...
    /**
     * Reflective manipulation of a URI field to work around the URI parser, because all fields are validated even on the full qualified URI constructor.
     *
